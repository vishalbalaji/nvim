...
function M.error(msg)
	error(string.format("(mini.hipatterns) %s", msg), 0)
end

function M.wrap_pattern_with_filter(pattern, filter)
	return function(...)
		if not filter(...) then
			return nil
		end
...
function H.shorthand(opts)
	...
	if style == "inline" and vim.fn.has("nvim-0.10") == 0 then
		M.error('Style "inline" in `H.shorthand()` requires Neovim>=0.10.')
	end

	local pattern = "()#%x%x%x()%f[^%x%w]"
	...
	return {
		pattern = M.wrap_pattern_with_filter(pattern, opts.filter),
		group = function(_, _, data)
			...
			return MiniHipatterns.compute_hex_color_group(hex_color, hl_style)
		end,
...
function H.tailwind(opts)
	...
	local style = opts.style
	if style == "inline" and vim.fn.has("nvim-0.10") == 0 then
		M.error('Style "inline" in `H.tailwind()` requires Neovim>=0.10.')
	end

	local pattern = ""
	if opts.style == "full" or opts.style == "inline" then
		pattern = "%f[%w:-]()[%w:-]+%-[a-z%-]+%-%d+()%f[^%w:-]"
	elseif opts.style == "compact" or opts.style == "inline_compact" then
		pattern = "%f[%w:-][%w:-]+%-()[a-z%-]+%-%d+()%f[^%w:-]"
	end

	local hl_style = ({ full = "bg", compact = "bg", inline = "fg", inline_compact = "fg" })[style] or "bg"

	local extmark_opts = { priority = opts.priority }
	if opts.style == "inline" or opts.style == "inline_compact" then
		local priority, inline_text = opts.priority, opts.inline_text
		---@diagnostic disable:cast-local-type
		extmark_opts = function(_, _, data)
			local virt_text = { { inline_text, data.hl_group } }
